...
void rst::rasterizer::rasterize_triangle(
    ...
    for (x = left; x <= right; x++) {
        for (y = bottom; y <= top; y++) {
            int   cnt      = 0;
            float minz     = INFINITY;
            float minalpha = INFINITY, minbeta = INFINITY, mingamma = INFINITY;
            // 未开启msaa
            ...
            if (depth_buf[get_index(x, y)] > minz) {
                auto interpolated_color =
                        interpolate(minalpha, minbeta, mingamma, t.color[0],
                                    t.color[1], t.color[2], 1);
                auto interpolated_normal =
                        interpolate(minalpha, minbeta, mingamma, t.normal[0],
                                    t.normal[1], t.normal[2], 1)
                                .normalized();
                auto interpolated_texcoords = interpolate(
                        minalpha, minbeta, mingamma, t.tex_coords[0],
                        t.tex_coords[1], t.tex_coords[2], 1);
                auto interpolated_shadingcoords =
                        interpolate(minalpha, minbeta, mingamma, view_pos[0],
                                    view_pos[1], view_pos[2], 1);

                fragment_shader_payload payload(
                        interpolated_color, interpolated_normal,
                        interpolated_texcoords, texture ? &*texture : nullptr);
                payload.view_pos = interpolated_shadingcoords;
                auto pixel_color = fragment_shader(payload);
                set_pixel(Vector2i(x, y), (pixel_color * cnt) / 4.0);
